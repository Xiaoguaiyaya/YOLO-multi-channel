...
class ProfileModels:
    ...
    def profile_tensorrt_model(self, engine_file: str, eps: float = 1e-3):
        ...
        channels = getattr(model.model, "channels", 3)  # exported models carry input channels in their metadata
        input_data = np.zeros((self.imgsz, self.imgsz, channels), dtype=np.uint8)  # use uint8 for Classify
