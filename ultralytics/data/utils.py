...
def check_cls_dataset(dataset: str | Path, split: str = "") -> dict[str, Any]:
    ...
    Returns:
        (dict[str, Any]): A dictionary containing the following keys:
            ...
            - 'channels' (int): The number of image channels, detected from a sample image.
    """
    ...
    # Detect input channels from one sample image to support multispectral and grayscale datasets
    channels = 3
    if sample := next((f for f in train_set.rglob("*.*") if f.suffix[1:].lower() in IMG_FORMATS), None):
        im = imread(sample)
        if im is not None and im.ndim == 3:
            channels = im.shape[2]

    return {"train": train_set, "val": val_set, "test": test_set, "nc": nc, "names": names, "channels": channels}
