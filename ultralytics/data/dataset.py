...
from .augment import (
    ...
    classify_multispectral_transforms,
    classify_transforms,
    v8_transforms,
)
...
class ClassificationDataset:
    ...
    def __init__(self, root: str, args, augment: bool = False, prefix: str = ""):
        ...
        # Channel-agnostic transforms for multispectral (>4-channel) images that PIL/torchvision cannot represent
        self.ms_transforms = classify_multispectral_transforms(
            size=args.imgsz,
            scale=scale,
            hflip=args.fliplr,
            vflip=args.flipud,
    ...
    def __getitem__(self, i: int) -> dict:
        ...
        if self.cache_ram:
            ...
            if not fn.exists():  # load npy
                np.save(fn.as_posix(), imread(f), allow_pickle=False)
            im = np.load(fn)
        else:  # read image
            im = imread(f)  # BGR
        if im.ndim == 2:
        ...
        if im.shape[2] in {3, 4}:  # standard grayscale/RGB/BGRA image convertible via PIL
            sample = self.torch_transforms(Image.fromarray(cv2.cvtColor(im, cv2.COLOR_BGR2RGB)))
        else:  # multispectral: bypass PIL with a channel-agnostic CHW tensor pipeline
            sample = self.ms_transforms(torch.from_numpy(np.ascontiguousarray(im.transpose(2, 0, 1))))
    ...
    def cache_images(self) -> None:
        ...
        with ThreadPool(NUM_THREADS) as pool:
            ims = list(
                TQDM(
                    pool.imap(lambda s: imread(s[0]), self.samples),
