...
def classify_multispectral_transforms(
    size: tuple[int, int] | int = 224,
    ...
    """Create channel-agnostic classification transforms for multispectral images with more than 4 channels.

    Unlike `classify_transforms()` and `classify_augmentations()`, which operate on PIL images and therefore only
    support up to 4 channels, these transforms accept a uint8 CHW torch.Tensor of any channel count and skip all
    ...
    Args:
        ...
        vflip (float): Probability of vertical flip (training only).
        erasing (float): Probability of random erasing (training only).
        interpolation (str): Interpolation method of either 'NEAREST', 'BILINEAR' or 'BICUBIC'.
        augment (bool): If True return training transforms (random resized crop + flips + erasing), else evaluation
            transforms (resize shortest edge + center crop).

    Returns:
        (torchvision.transforms.Compose): A composition of torchvision transforms operating on CHW tensors.

    Examples:
        >>> import numpy as np
        >>> transforms = classify_multispectral_transforms(size=224)
        >>> im = np.random.randint(0, 255, (240, 320, 7), dtype=np.uint8)  # HWC
        >>> sample = transforms(torch.from_numpy(im.transpose(2, 0, 1).copy()))  # CHW tensor in, CHW out
    """
    import torchvision.transforms as T  # scope for faster 'import ultralytics'
    ...
    if augment:
        ...
        if hflip > 0.0:
            tfl.append(T.RandomHorizontalFlip(p=hflip))
        if vflip > 0.0:
            tfl.append(T.RandomVerticalFlip(p=vflip))
        tfl += [T.ConvertImageDtype(torch.float), T.RandomErasing(p=erasing, inplace=True)]
    else:
        resize = size if isinstance(size, (tuple, list)) else size  # scalar resizes shortest edge preserving aspect
        tfl = [T.Resize(resize, interpolation=interpolation), T.CenterCrop(size), T.ConvertImageDtype(torch.float)]
    return T.Compose(tfl)
