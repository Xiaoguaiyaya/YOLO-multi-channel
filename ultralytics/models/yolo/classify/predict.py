...
from ultralytics.data.augment import classify_multispectral_transforms, classify_transforms
...
class ClassificationPredictor(BasePredictor):
    ...
    def preprocess(self, img):
        ...
        if not isinstance(img, torch.Tensor):
            ...
            for im in img:
                if im.ndim == 2:
                    im = im[..., None]
                if im.shape[2] in {3, 4}:  # standard grayscale/RGB/BGRA image convertible via PIL
                    imgs.append(self.transforms(Image.fromarray(cv2.cvtColor(im, cv2.COLOR_BGR2RGB))))
                else:  # multispectral: bypass PIL with a channel-agnostic CHW tensor pipeline
                    if not hasattr(self, "ms_transforms"):
                        self.ms_transforms = classify_multispectral_transforms(self.imgsz)
                    imgs.append(self.ms_transforms(torch.from_numpy(np.ascontiguousarray(im.transpose(2, 0, 1)))))
    ...
    def postprocess(self, preds, img, orig_imgs):
        ...
        if not isinstance(orig_imgs, list):  # Input images are a torch.Tensor, not a list
            ...
            if orig_imgs.shape[-1] == 3:  # multispectral bands keep their trained order, only RGB flips back to BGR
