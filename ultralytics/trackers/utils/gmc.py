...
def _to_gray(frame: np.ndarray) -> np.ndarray:
    """Convert a frame with any channel count to grayscale for feature-based motion estimation."""
    if frame.ndim == 2:
        return frame
    c = frame.shape[2]
    if c in {3, 4}:
        return cv2.cvtColor(frame, cv2.COLOR_BGR2GRAY)
    return frame.mean(axis=2).astype(frame.dtype)  # multispectral: band-average luminance
