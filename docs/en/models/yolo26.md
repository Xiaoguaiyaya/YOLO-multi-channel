...
## 多光谱输入（Multispectral Input）

YOLO26 支持任意通道数的图像输入（如 RGB + NIR + RedEdge + 热红外），不再局限于 3 通道 RGB。支持范围覆盖训练、验证、推理、跟踪与导出，适用于全部任务。

### 数据格式

图像存为多页 TIFF，每个波段一页（`cv2.imwritemulti` 约定）。全数据集波段顺序必须一致，网络按文件顺序原样接收。JPG/PNG 保持原生通道限制，多光谱数据请使用 TIFF。

可使用 `ultralytics.data.converter` 中的 `convert_to_multispectral` 工具从 RGB 图像合成 N 波段 TIFF 用于测试：
...
分类数据集使用 ImageFolder 目录结构，无需 `channels` 字段——自动从首张样本图像探测通道数。

### 训练

按常规流程训练即可，模型输入层按 `channels` 波段数自动构建：

```python
from ultralytics import YOLO

model = YOLO("yolo26n.yaml")  # 模型架构
model.train(data="ms.yaml", epochs=100, imgsz=640)
```

从 COCO 预训练权重（`yolo26n.pt`）微调时，除首层卷积外全部权重迁移；首层预训练的 3 波段卷积核被部分拷贝进新的 N 波段层（其余波段卷积核从头训练）。

增强说明：几何类增强（mosaic、mixup、翻转、透视变换）对任意通道数生效；RGB 专属增强（HSV 抖动、颜色抖动、Albumentations 管线、BGR 翻转）对多光谱图像自动跳过。

### 推理

直接对多光谱 TIFF 进行预测——通道数从 checkpoint 自动恢复：

```python
model = YOLO("best.pt")
results = model("path/to/image.tiff")
results[0].orig_img.shape  # (H, W, 7)，波段顺序保持不变
```

NumPy 数组可直接以 `(H, W, C)` 形式传入任意通道数；多光谱输入的波段序不会被重排（BGR↔RGB 翻转仅作用于 3 通道数据）。

### 输出物

- 标注预测图、验证网格图与混淆矩阵渲染前三个波段。
- `save_crop` 导出检测目标的 3 波段 JPG 裁剪图。
- 跟踪开箱即用；运动补偿会将帧转为灰度亮度（大于 4 通道取波段均值）。

### 导出

ONNX、TensorRT、OpenVINO、TorchScript 和 Ascend 导出会在元数据中携带输入通道数并接受 N 通道输入。CoreML 图像输入受 coremltools 限制仅支持 1-3 通道，更宽输入会显式报错。

### 已知限制

- YOLOE / YOLO-World 提示编码器假设 RGB 3 通道输入。
- SAM / FastSAM 模型族为 3 通道设计。
- 分类 auto_augment 策略（RandAugment 等）仅作用于不超过 4 通道的图像；多光谱样本使用几何增强加随机擦除。
